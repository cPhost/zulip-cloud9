...
# check for the venv
from zulip import sanity_check
...
FILE = path.abspath(ZULIP_DIR + '/tools/run-dev.py')
...
if options.interface is None:
    ...
    if user_name in ["vagrant", "zulipdev"]:
        ...
        # exposing certain guest ports (by default just 8080) to the
...
def get_cmd(cmd):
  rel_path = path.relpath(ZULIP_DIR + '/' + cmd)
...
subprocess.check_call(get_cmd('tools/clean-repo'))

if options.clear_memcached:
    print("Clearing memcached ...")
    subprocess.check_call(get_cmd('scripts/setup/flush-memcached'))
...
# zulip/urls.py.
cmds = [[get_cmd('/tools/compile-handlebars-templates'), 'forever'],
        [get_cmd('manage.py'), 'runserver'] +
        manage_args + runserver_args + ['0.0.0.0:%d' % (django_port,)],
        ['env', 'PYTHONUNBUFFERED=1', get_cmd('manage.py'), 'runtornado'] +
        manage_args + ['0.0.0.0:%d' % (tornado_port,)],
        [get_cmd('/tools/run-dev-queue-processors')] + manage_args,
        ['env', 'PGHOST=0.0.0.0',  # Force password authentication using .pgpass
         get_cmd('/puppet/zulip/files/postgresql/process_fts_updates')],
        [get_cmd('manage.py'), 'deliver_scheduled_messages']]
if options.test:
